...
if (fs.existsSync(stepsDir)) {
    ...
    for (const folder of folders) {
        // Specifically include steps 1170, 1171, 1172, 1173
...
// Regex to capture escaped json elements: \"id\":\"...\",\"assetId\":\"...\" from steps 1170-1173
...
console.log('==================================================');
console.log(`📦 LOAD THÀNH CÔNG DỮ LIỆU CỰC NGUYÊN BẢN: ${itemsArray.length} PURE PETS TỪ DIRECTORY.PETS CỦA GAME!`);
console.log('==================================================\n');

// Clean destination directory completely
if (fs.existsSync(outputDir)) {
    const oldFiles = fs.readdirSync(outputDir);
    for (const f of oldFiles) {
        fs.unlinkSync(path.join(outputDir, f));
    }
    console.log(`🧹 Đã làm sạch 100% thư mục Pets để xóa toàn bộ các file rác cũ!\n`);
} else {
...
async function startFastBatchDownload() {
    ...
    console.log('\n==================================================');
    console.log('🎉 TỔNG KẾT BỘ ẢNH 100% PURE PETS PET SIMULATOR 99:');
    console.log(`⏱️ Thời gian hoàn tất: ${elapsedSeconds} Giây!`);
    console.log(`• Tải mới thành công: ${downloadedCount} file PNG (100% Pure Pets)`);
