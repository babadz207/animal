...
for (const file of files) {
    if (!validPetFiles.has(file)) {
        fs.unlinkSync(path.join(petsDir, file));
        console.log(`  🗑️ XÓA TRIỆT ĐỂ FILE RÁC: ${file}`);
        deletedCount++;
    } else {
        keptCount++;
    }
...
console.log('\n==================================================');
console.log('🎉 ĐÃ DỌN SẠCH CHUẨN 100% TRONG WINDOWS EXPLORER:');
console.log(`• Số File Rác Đã Xóa Khỏi Thư Mục: ${deletedCount} file PNG`);
console.log(`• Số Pet Thật Giữ Lại: ${keptCount} file PNG`);
