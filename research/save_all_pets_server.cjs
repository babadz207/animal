...
async function startFastBatchDownload(itemsArray) {
    console.log(`🚀 [TẢI ĐA LUỒNG 30 WORKERS] Bắt đầu tải TOÀN BỘ ${itemsArray.length} Pet Icons...\n`);
    ...
    console.log('\n==================================================');
    console.log('⚡ TỔNG KẾT QUÁ TRÌNH TẢI TOÀN BỘ 3,494 PET SIÊU TỐC:');
    ...
    if (req.method === 'POST') {
        ...
        req.on('end', () => {
            try {
                ...
                allPetsList = allPetsList.concat(petsChunk);
                console.log(`📥 Nhận đợt dữ liệu mới: +${petsChunk.length} Pets (Tổng hiện tại: ${allPetsList.length} Pets)`);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ status: 'ok', currentTotal: allPetsList.length }));

                if (req.headers['x-is-final'] === 'true') {
                    console.log(`🎉 ĐÃ NHẬN ĐỦ TOÀN BỘ ${allPetsList.length} PETS TỪ GAME! BẮT ĐẦU TẢI 30 LUỒNG...\n`);
                    startFastBatchDownload(allPetsList);
                }
            } catch (err) {
        ...
        res.writeHead(200);
        res.end('All Pets Receiver Server Running');
    }
});

server.listen(19999, '0.0.0.0', () => {
    console.log('🌐 Server Nhận Dữ Liệu Toàn Bộ 3,494 Pets sẵn sàng tại cổng 19999!');
});
