...
async function processPetsChunk(itemsArray, chunkName) {
    console.log(`\n🚀 [TẢI SIÊU TỐC 30 WORKERS - ${chunkName}] Bắt đầu tải ${itemsArray.length} Pet Icons...`);
    ...
    console.log(`✅ [${chunkName} HOÀN THÀNH trong ${elapsedSeconds}s]: Tải mới ${downloadedCount} PNG, Đã có sẵn ${skippedCount} PNG.`);
    return { downloadedCount, skippedCount, failedCount };
}

module.exports = { processPetsChunk };
