...
const inputPath = path.join(__dirname, 'ps99_pet_assets_map.json');
...
const itemRegex = /"ID":\s*"([^"]+)"[^{}]*?"AssetId":\s*"(\d+)"/g;
...
async function startBatchDownload() {
    console.log('🚀 Bắt đầu tự động tải toàn bộ Icon Pet PNG về máy...\n');
    let totalSuccess = 0;
    let totalSkipped = 0;
    let totalFailed = 0;
                ...
                for (const itemObj of batch) {
                    ...
                    try {
                        ...
                        if (status === 'downloaded') {
                            totalSuccess++;
                            console.log(`  ✅ Tải thành công Pet: ${filename}`);
                        } else {
                            totalSkipped++;
                        }
                    } catch (err) {
