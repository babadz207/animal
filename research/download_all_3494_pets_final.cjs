...
// Extract {"id":"...","assetId":"..."} JSON string elements
const itemRegex = /\\"id\\":\\"([^"\\]+)\\",\\"assetId\\":\\"(\d+)\\"/g;
...
console.log('==================================================');
console.log(`📦 BẮT ĐẦU TẢI CHUẨN 100% TOÀN BỘ ${itemsArray.length} PETS TRONG GAME!`);
console.log('==================================================\n');
...
async function startFastBatchDownload() {
    console.log(`🚀 [SIÊU TỐC ĐA LUỒNG 30 WORKERS] Bắt đầu tải TOÀN BỘ ${itemsArray.length} PETS CỦA GAME...\n`);
        ...
        try {
            ...
            if (status === 'downloaded') {
                ...
                if (downloadedCount % 100 === 0 || downloadedCount === downloadQueue.length) {
                    console.log(`  ⚡ [${downloadedCount}/${downloadQueue.length}] ✅ Tải xong Pet: ${task.filename}`);
                }
            } else {
                skippedCount++;
            }
    ...
    console.log('\n==================================================');
    console.log('⚡ TỔNG KẾT BẮT ĐẦU TẢI ĐỦ 100% PET SIMULATOR 99:');
