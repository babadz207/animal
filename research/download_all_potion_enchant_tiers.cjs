...
const inputPath = path.join(__dirname, 'ps99_all_tiers_map.json');
...
// Regex match for ID, AssetId, Category
const itemRegex = /ID="([^"]+)"[^{}]*?AssetId="(\d+)"[^{}]*?Category="([^"]+)"|AssetId="(\d+)"[^{}]*?ID="([^"]+)"[^{}]*?Category="([^"]+)"/g;
...
while ((match = itemRegex.exec(rawText)) !== null) {
    ...
    const category = match[3] || match[6];
    if (id && assetId && category) {
        itemsMap.set(`${id}_${assetId}`, { id, assetId, category });
    }
}

const itemsArray = Array.from(itemsMap.values());
console.log(`📦 Tìm thấy tổng cộng ${itemsArray.length} Icon của từng Tier Potion & Enchant!`);
...
async function startBatchDownload() {
    console.log('🚀 Bắt đầu tự động tải toàn bộ Icon Potion & Enchant từ Tier 1 đến Tier 10...\n');
    let totalSuccess = 0;
    let totalSkipped = 0;
    let totalFailed = 0;
    ...
    console.log('\n==================================================');
    console.log('🎉 TỔNG KẾT QUÁ TRÌNH TẢI ICON POTION & ENCHANT TIER 1 ➔ 10:');
