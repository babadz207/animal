...
console.log(`📦 Đã load ${itemsArray.length} Pet Icon độc nhất!`);
...
async function startFastBatchDownload() {
    console.log('🚀 [SIÊU TỐC DÙNG ĐA LUỒNG - 30 PARALLEL CONCURRENCY] Bắt đầu tải Pet Icons...\n');
    ...
    console.log(`📡 Đang truy vấn Roblox API cho ${batches.length} đợt (mỗi đợt 100 Asset IDs)...`);
    const downloadQueue = [];
    ...
    await runParallelPool(downloadQueue, 30, async (task) => {
        try {
            ...
            if (status === 'downloaded') {
                downloadedCount++;
                if (downloadedCount % 20 === 0 || downloadedCount === downloadQueue.length) {
                    console.log(`  ⚡ [${downloadedCount}/${downloadQueue.length}] ✅ Tải thành công: ${task.filename}`);
                }
            } else {
                skippedCount++;
            }
