...
// Regex to capture {id="...",assetId="..."} in Luau table dumps
const itemRegex = /\{id="([^"]+)",assetId="(\d+)"\}|\{assetId="(\d+)",id="([^"]+)"\}/g;
...
console.log(`📦 ĐÃ LOAD THÀNH CÔNG TỔNG CỘNG ${itemsArray.length} PET ICON ĐỘC NHẤT TRONG GAME!`);
