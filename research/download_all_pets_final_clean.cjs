...
// Regex matching {"id":"...","assetId":"..."} in steps and logs
...
while ((match = itemRegex.exec(allText)) !== null) {
    ...
    // STRICT NON-PET FILTERING:
    // 1. Exclude any ID ending with single quote (Fruit item like Apple', Banana', Candycane', Orange', Pineapple', Watermelon')
    // 2. Exclude non-pet keywords
    if (id.endsWith("'") || id.includes("Card") || id.includes("Boost") || id.includes("Sprinkler") ||
        id.includes("Flag") || id.includes("Potion") || id.includes("Enchant") || id.includes("Key") ||
        id.includes("Axe") || id.includes("Shovel") || id.includes("Orb") || id.includes("Dice") ||
        id.includes("Token") || id.includes("Ticket") || id.includes("TNT") || id.includes("Bundle")) {
        continue;
    }
...
console.log('==================================================');
console.log(`📦 BẮT ĐẦU TẢI BỘ ẢNH 100% PURE PETS (${itemsArray.length} LOÀI PET CHUẨN)!`);
console.log('==================================================\n');
...
async function startFastBatchDownload() {
    console.log(`🚀 [SIÊU TỐC ĐA LUỒNG 30 WORKERS] Bắt đầu tải ${itemsArray.length} PURE PETS...\n`);
